...
root_agent = Agent(
    ...
    **Required Conversational Workflow:**
    ...
    8.  **Create Podcast Script (Internal Step):** After saving the report, you MUST convert the structured `AINewsReport` data into a natural, conversational podcast script between two hosts, 'Ravi' (enthusiastic) and 'Geetha' (analytical). One has a British accent, the other Indian. The script MUST cover all key stories and insights from the report in an engaging manner.
